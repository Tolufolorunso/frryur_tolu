...
class Venue(db.Model):
    ...
    def __repr__(self):
        return f'<Venue ID: {self.id}, name: {self.name}, city: {self.city}, address: {self.address}>'
...
def venues():
    ...
    values = Venue.query.order_by(Venue.state, Venue.city, Venue.name).all()
    data = []
    prev_city = None
    prev_state = None
    for value in values:
        if value.city != prev_city or value.state != prev_state:
            data.append(
                {'city': value.city, 'state': value.state, 'venues': []}
            )
...
def search_artists():
    ...
    search_term = request.form.get('search_term').lower()
    # data = Artist.query.filter(Artist.name).all()
